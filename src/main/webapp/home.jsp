...
<html>
    ...
    <body>
        ...
        <!-- the forEach jsp tag wasn't working for me (different version maybe) so I did it this way instead 
        ...
        for (OrigamiKit product : products) {
            ...
            + "<form action='ProductServlet' id='" + id + "'>"
            + "<input type='hidden' name='product' value='" + id + "'>"
            + "<a href='#' onclick='submitProductForm(" + id + ")'>"
            ...
            + "<a class='text-dark' class='text-info href='#' onclick='submitProductForm(" + id + ")'>"
            + "<h4 class='card-title'>" + product.getName() + "</h4>"
            + "</a>"
            + "<ul class='list-unstyled mt-3 mb-4'>"
            + "<li><a class='text-dark' href='#' onclick='submitProductForm(" + id + ")'><h5>€" + product.getPrice() + "</h5></a></li>"
            + "<li><p class='card-text mb-5'>" + product.getDesc() + "</p></li>"
            + "</form>"
            + "<form action='AddToCart'>"
            + "<button type='submit' name='product' value='" + id
            + "' class='mt-auto btn btn-dark btn-lg mb-4 mt-2' style='position: absolute; bottom: 0;'>Add to cart</button>"
            + "</form>"
            + "</div>"
            + "</div>");
        }
